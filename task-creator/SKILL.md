...
7. **Default the Description to BA (business-facing) language, not technical/code language.** Write Context, Current Problem, Desired Outcome, Purpose, and Scope of Change in terms a non-technical stakeholder (PM, BA, CS lead) can follow — describe business impact, user-facing behavior, and affected functional areas, not file paths, class names, function names, or code snippets. This is the default for every task drafted by this skill, not something to ask about up front. Only include technical identifiers (file paths, class/function names, table/column names) when the user explicitly asks for a more technical description, or explicitly says the ticket is for a dev-only board/audience — see Step 3 for how to phrase each section either way.
...
**Default to BA (business-facing) wording in every section below** — write for a reader who doesn't read code: describe what happens/breaks from the user's or business's point of view, not which function or file is involved. Reserve file paths, class/function names, table/column names, and code snippets for a technical-detail case: the user explicitly asks for a technical description, or explicitly says this is a dev-only ticket/board. Even then, keep the narrative sections (Context, Current Problem, Desired Outcome, Purpose) business-facing and put the technical identifiers in Scope of Change only — that section is the one place technical detail is expected once asked for.

1. **Context** — The current business situation that led to this task: what feature/screen/workflow this is part of, and who relies on it. Helps the reader (PM, reviewer, another dev) understand "why" the task exists without needing to know the implementation.
2. **Current Problem** — What is specifically wrong/missing/suboptimal, described as user-visible or business-visible symptoms (what the user sees, what breaks, what's blocked). For bugs: include reproduction steps in terms of user actions, plus environment/timing if relevant — avoid stack traces or internal error codes unless the user asked for technical detail.
3. **Desired Outcome** — The state after the task is completed, stated clearly and measurably where possible (short acceptance criteria), phrased as an outcome the business/user experiences, not an implementation detail.
4. **Purpose** — What problem this task solves for the user/system/team; why it's worth doing.
5. **Scope of Change** — The functional areas/workflows expected to be touched (e.g. "new-request creation flow", "historical data cleanup", "the filter on the admin orders screen"), in BA language. If unclear, write "Needs further investigation once started" instead of leaving it blank. If you have file/code access to the relevant repository, it's worth briefly inspecting the actual code (e.g. `view`/`bash` a few relevant files or directories) to confirm which functional areas are really involved, rather than relying only on the user's description — this makes both this section and the Step 4 estimate more accurate, even though the write-up stays in business language. Don't do a full deep audit; a quick, targeted look is enough. Only list concrete file paths/class names here if the user asked for a technical description.
